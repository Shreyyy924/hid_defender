...
class TestIntegration:
    ...
    def test_tc18_continuous_monitoring(self, temp_whitelist_file, temp_log_file):
        ...
        with patch("hid_defender.device_validator.WHITELIST_PATH", str(temp_whitelist_file)), patch(
            "hid_defender.logging_setup.LOG_PATH", str(temp_log_file)
        ), patch("hid_defender.config.WHITELIST_PATH", str(temp_whitelist_file)), patch(
            "hid_defender.config.LOG_PATH", str(temp_log_file)
        ):
    ...
    def test_tc20_end_to_end_workflow(self, temp_whitelist_file, temp_log_file):
        ...
        with patch("hid_defender.device_validator.WHITELIST_PATH", str(temp_whitelist_file)), patch(
            ...
            "hid_defender.alert_system.subprocess.run"
        ) as mock_alert:
            ...
            show_alert({"name": attack_device["description"], "id": attack_device["hardware_id"]})
    ...
    def test_attack_device_detection_integration(self, temp_whitelist_file):
        ...
        with patch("hid_defender.device_validator.WHITELIST_PATH", str(temp_whitelist_file)):
            ...
            attack_devices = [
                {
                    "hardware_id": "VID_2E8A&PID_0003",
                    "vendor": "Raspberry Pi",
                    "description": "Pico",
                },
                {
                    "hardware_id": "VID_2341&PID_8036",
                    "vendor": "Arduino",
                    "description": "Leonardo",
                },
                {
                    "hardware_id": "VID_16C0&PID_0483",
                    "vendor": "Teensy",
    ...
    def test_cross_module_integration(self, temp_whitelist_file, temp_log_file):
        ...
        with patch("hid_defender.config.WHITELIST_PATH", str(temp_whitelist_file)), patch(
            "hid_defender.config.LOG_PATH", str(temp_log_file)
        ):
            # Import all modules
            from hid_defender.logging_setup import init_logger
