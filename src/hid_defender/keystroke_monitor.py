...
class KeystrokeMonitor:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None) -> None:
        ...
        try:
            ...
            self.threshold = 100.0  # Fallback threshold
    ...
    def register_device_connection(self, device_info: Dict[str, Any]) -> None:
        ...
        self.logger.info(f"Tracking suspicious device for first-input delay: {name} ({device_id})")
    ...
    def start(self) -> None:
        ...
        if not PYNPUT_AVAILABLE or keyboard is None:
            self.logger.warning(
                "Keystroke monitoring requires pynput. Install with: pip install pynput"
            )
            return
        ...
        try:
            ...
            if IS_MACOS:
                ...
                if "not trusted" in stderr_output.lower() or not self.listener.is_alive():
                    ...
                    self.listener = None
                    self.logger.warning(
                        "⚠️  Keystroke monitoring disabled - requires accessibility permissions"
                    )
                    self.logger.warning(
                        "   On macOS, grant access via: System Preferences → Privacy & Security → Accessibility"
                    )
                    self.logger.warning(
                        "   Add your Terminal or Python to the list, then restart this script"
                    )
